...
public class ExcelUtil<T> {
	...
	public void exportExcel(String[] headers, Collection<T> dataset,
			OutputStream out,InputStream input,int cindex) {
		try {
			exportExcel("sheet1", headers, dataset, out,input, "yyyy-MM-dd",cindex);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
...
//		comment.setAuthor("leno");
	...
	@SuppressWarnings({ "unchecked", "deprecation", "resource", "rawtypes" })
	public void exportExcel(String title, String[] headers,
			Collection<T> dataset, OutputStream out,InputStream input, String pattern,int cindex) throws IOException {
		// 声明一个工作薄
